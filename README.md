...
(vsum v1 v2 ...)               ; R0 = v1+v2+... → VSUM_IMM за одну инструкцию переменной длины.
(vmul v1 v2 ...)               ; R0 = v1*v2*...
(vsub v1 v2 ...)               ; R0 = v1-v2-...
```
...
| **Память данных** | Чтение и запись. Содержит строковые литералы, переменные, временные ячейки компилятора, стек. Адрес доступа формируется на лету мультиплексором адреса (без отдельного регистра). |
...
- **Регистры и флаги:** `R0` (аккумулятор, регистр возврата), `R1` (временный), `SP` (стек, растёт вниз от `0x0F00`), `PC` (счётчик команд). Флаги в `FLAGS`: `ZF` (результат == 0), `NF` (результат < 0), `IF` (разрешение прерываний). Отдельного адресного регистра (MAR) нет: адрес доступа к памяти данных формируется комбинационно мультиплексором адреса и подаётся в память в том же такте.
- **Память и адресация:** Гарвардская архитектура; машинное слово — 32 бита; адресация пословная. Три режима: непосредственная (`LOAD R0, #42`), прямая (`LOAD R0, [0x0400]`), косвенная регистровая (`LOAD R0, [R1]`). Доступ к памяти данных — `signal_data_read(addr)`/`signal_data_write(addr, value)`, где `addr` приходит шиной с мультиплексора адреса.
...
- **Прерывания (trap):** IRQ поднимается контроллером при поступлении символа. Условие принятия: `IRQ=1 AND IF=1 AND NOT in_interrupt`. Аппаратная секвенция занимает 6 тактов: такт принятия (`INT_ACCEPT`: подтверждение IRQ и фиксация `current_instr_pc`→PC), затем 4 такта сохранения на стек `R0`, `R1`, `current_instr_pc`, `FLAGS`, и такт загрузки PC из вектора `[0x0002]` со сбросом `IF`. Сохраняется именно `current_instr_pc` — адрес **начала** прерванной инструкции, поэтому после `IRET` она **перезапускается**. Вложенные прерывания невозможны: `IF=0` на всё время ISR.
- **CISC:** инструкции переменной длины (1 или 2 слова; варарг — N+1 слов); арифметические операции работают с регистром и ячейкой памяти за одну инструкцию (`ADD R0, [addr]`); варарг-инструкции `VSUM_IMM`/`VMUL_IMM`/`VSUB_IMM` принимают произвольное число immediate-операндов.
...
| `0x70` | `EI` | 1 | **2** | `IF = 1` |
| `0x71` | `DI` | 1 | **2** | `IF = 0` |
| `0x80` | `SUM R0, n; v1..vN` | N+1 | **N+1** | `R0 = v1+v2+...+vN`; ZF, NF |
| `0x81` | `MUL R0, n; v1..vN` | N+1 | **N+1** | `R0 = v1*v2*...vN`; ZF, NF |
| `0x82` | `SUB R0, n; v1..vN` | N+1 | **N+1** | `R0 = v1-v2-...-vN`; ZF, NF |
...
**Тактовый журнал** (`--log`) — одна строка на такт, поля разделены ` | `. Содержит:
- состояние регистров (`R0`, `R1`, `SP`, `PC`) и флагов (`Z`, `N`, `I`), сигнал `IRQ`;
...
Формат строки:
```
TICK: 0042  | FETCH1  | PC: 0012  | OP: LOAD R0, #0x00FF  | R0: 00000000  | R1: 00000000  | SP: 0EFF  | Z: 0  | N: 0  | I: 1  | IRQ: 0  | IR: [11000000, 000000FF]  | ACT: LOAD R0, #255
```
...
| 9 | [`varargs.yaml`](tests/golden/varargs.yaml) | – | результаты `vsum`/`vmul`/`vsub` | CISC варарг-инструкции `VSUM_IMM`, `VMUL_IMM`, `VSUB_IMM` |
...
| 12 | [`cat_overrun.yaml`](tests/golden/cat_overrun.yaml) | `input/cat_overrun.txt` | `C` | физическое поведение прерываний: три ввода подряд (такты 100/101/102) → overrun, echo последнего символа |
